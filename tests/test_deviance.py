...
def test_dense_top_n():
    ...
    ad = AnnData(X=X)
    scrypy.highly_deviant_features(ad, n_top_features=2, inplace=True)
...
def test_sparse_matches_dense():
    ...
    scrypy.highly_deviant_features(ad_d, n_top_features=3, inplace=True)
    scrypy.highly_deviant_features(ad_s, n_top_features=3, inplace=True)
...
def test_batch_key():
    ...
    ad = AnnData(X=X)
    ad.obs['b'] = ['a', 'a', 'b']
    scrypy.highly_deviant_features(ad, n_top_features=2, batch_key='b', inplace=True)
    assert ad.var['highly_variable'].sum() == 2
...
def test_inplace_false_subset():
    ...
    ad = AnnData(X=X)
    df = scrypy.highly_deviant_features(ad, n_top_features=1, inplace=False, subset=True)
    assert len(df) == 1
...
def test_n_top_features_exceeds_n_vars():
    X = np.array([[1, 2], [3, 4]], dtype=float)
    ad = AnnData(X=X)
    with pytest.warns(UserWarning, match='n_top_features'):
        scrypy.highly_deviant_features(ad, n_top_features=10, inplace=True)
